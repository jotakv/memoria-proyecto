# Modelo de viabilidad

**Estado:** [PENDIENTE] de entradas aprobadas por el promotor

No se incluyen previsiones numéricas porque faltan capacidad, costes, fiscalidad, inversión y supuestos comerciales documentados.

## Entradas necesarias

| Variable | Unidad | Valor | Origen requerido |
|---|---|---:|---|
| Días facturables por mes | días | Pendiente | Decisión del promotor |
| Capacidad facturable | horas/mes | Pendiente | Calendario operativo |
| Mezcla de servicios | % o unidades | Pendiente | Hipótesis aprobada |
| Precio medio por servicio | € | Pendiente | HYP-A01 a HYP-A03 y alcance |
| Captación y conversión | oportunidades/clientes | Pendiente | Validación comercial |
| Costes fijos mensuales | €/mes | Pendiente | Contratos y presupuestos |
| Costes variables | €/proyecto o % | Pendiente | Licencias, cloud y colaboradores |
| Cotizaciones e impuestos | €/mes o % | Pendiente | Asesoría y fuentes oficiales |
| Inversión inicial | € | Pendiente | `PLAN_INVERSION.md` |
| Fondo de maniobra | € | Pendiente | Escenario de tesorería |
| Costes del local | €/mes/año | Pendiente | electricidad, agua, internet, comunidad, IBI, seguro y mantenimiento |
| Costes de adquisición/acondicionamiento | € | Pendiente | liquidaciones, ofertas e inspección |
| Retirada personal | €/mes | 800 | [CONFIRMADO por declaración]; solo salida personal de caja |

## Salidas a calcular

- cuenta de resultados mensual del primer año;
- tesorería de doce meses;
- margen por tipo de servicio;
- utilización de capacidad;
- punto de equilibrio;
- necesidad de fondo de maniobra;
- escenarios conservador, base y favorable con multiplicadores aprobados;
- sensibilidad a retrasos comerciales y consumo cloud.

## Regla de uso

No se calculará ni publicará facturación, rentabilidad o empleo hasta que las entradas tengan fuente y aceptación expresa. Los rangos de precio de la memoria no bastan para demostrar viabilidad.

El modelo anterior sin local no permite afirmar liquidez. Con aproximadamente 41.000 € de fuentes identificadas frente al presupuesto de referencia de 40.000 € para el precio, las proformas de 2.738,84 € y costes accesorios pendientes generan **BRECHA DE FINANCIACIÓN PENDIENTE**. Deben recalcularse amortización, punto de equilibrio y caja antes de adquirir.
